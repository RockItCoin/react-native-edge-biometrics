...
@ReactModule(name = EdgeBiometricsModule.NAME)
public class EdgeBiometricsModule extends ReactContextBaseJavaModule {
  public static final String NAME = "EdgeBiometrics";
  ...
  public void fetch(
    ...
    Log.d(NAME, method + ": " + url + " ");
    for (int i = 0; i < headers.size(); i++) {
      Log.d(NAME, "header: " + headers.getString(i));
    }
    try {
                    ...
                    switch (result.readState) {
                      ...
                      case READY:
                        if (result.value != null) {
                          ...
                          // No value was found. Fall back to password or fail silently, depending
                          // on
                          // your use case.
  ...
  private boolean isFingerprintAvailable() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      return ActivityCompat.checkSelfPermission(AppContext, Manifest.permission.USE_FINGERPRINT)
  ...
  private boolean checkHardwareSupport() {
    if (mStorage == null || mActivity == null || mWhorlwind == null) {
      mStorage =
          new SharedPreferencesStorage(getReactApplicationContext(), "co.airbitz.airbitz.storage");
        ...
        public void run() {
          ...
          if (mFingerprintStatus != null) {
            mFingerprintStatus.setTextColor(
                ContextCompat.getColor(mActivity, R.color.dark_text_hint));
            mFingerprintStatus.setText(R.string.fingerprint_hint);
          }
